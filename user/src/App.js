...
const AdminDashboard = ({ onLogout, userLevel, username }) => {
  ...
  const [questionDates, setQuestionDates] = useState({ fromDate: '', toDate: '' });
  ...
  const handleAddQuestion = async () => {
    ...
    if (questionDates.fromDate && questionDates.toDate) {
      if (new Date(questionDates.fromDate) > new Date(questionDates.toDate)) {
    ...
    try {
      if (selectedDepartment) {
        ...
        const response = await fetch(`${API_BASE_URL}/departments/${dept.URLSlug}/questions`, {
          ...
          body: JSON.stringify({
            ...
            fromDate: questionDates.fromDate || null,
            toDate: questionDates.toDate || null
          })
        });

        if (response.ok) {
          const result = await response.json();
          setSuccess(result.message); // Shows detailed activation status
        } else {
          const errorData = await response.json();
        ...
        if (response.ok) {
          const result = await response.json();
          setSuccess(result.message); // Shows how many activated vs scheduled
        } else {
          const errorData = await response.json();
      ...
      setQuestionDates({ fromDate: '', toDate: '' });
            ...
            <div className="mb-6">
              <div className="mb-4">
                ...
                <select
                  ...
                  className="w-full p-2 border rounded-md"
                >
                  <option value="">Select department (leave empty for all departments)</option>
                  {departments.map(dept => (
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Leave empty to send question to all departments at once
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    type="date"
                    value={questionDates.fromDate}
                    onChange={(e) => setQuestionDates({ ...questionDates, fromDate: e.target.value })}
                    className="w-full p-2 border rounded-md"
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    Question will activate on this date
                  </p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                  ...
                  <input
                    type="date"
                    value={questionDates.toDate}
                    onChange={(e) => setQuestionDates({ ...questionDates, toDate: e.target.value })}
                    className="w-full p-2 border rounded-md"
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    Question will deactivate after this date
                  </p>
                </div>
              </div>
                ...
                <div className="flex items-start">
                  ...
                  <div className="text-sm text-blue-800">
                    ...
                    <ul className="list-disc list-inside space-y-1 text-xs">
                      <li>Questions will auto-activate on the From Date</li>
                      <li>Questions will auto-deactivate after the To Date</li>
                      <li>Only one question per department can be active at a time</li>
                    </ul>
                  </div>
                </div>
              </div>
              ...
              >
                {loading ? 'Adding...' : selectedDepartment ? 'Add Question to Department' : 'Add Question to All Departments'}
              </button>
            </div>
              ...
              {questions.length === 0 ? (
                ...
                questions.map(q => {
                  ...
                  if (q.IsActive) {
                    ...
                    statusText = 'Active';
                  } else if (q.FromDate && new Date(q.FromDate) > today) {
                    statusColor = 'bg-yellow-100 text-yellow-800';
                    statusText = 'Scheduled';
                  } else if (q.ToDate && new Date(q.ToDate) < today) {
                  ...
                  return (
                    <div key={q.QuestionID} className="p-3 border rounded-md">
                      <div className="flex justify-between items-start">
                        ...
                        <span className={`px-2 py-1 text-xs rounded ml-2 ${statusColor}`}>
                          {statusText}
                        </span>
                      </div>
                      <div className="mt-2 text-sm text-gray-600 space-y-1">
                        <p>{dept?.Name || 'Unknown Department'}</p>
                        {q.FromDate && (
                          <p className="flex items-center">
                            <span className="font-medium mr-2">From:</span>
                            {new Date(q.FromDate).toLocaleDateString()}
                          </p>
                        )}
                        {q.ToDate && (
                          <p className="flex items-center">
                            <span className="font-medium mr-2">To:</span>
                            {new Date(q.ToDate).toLocaleDateString()}
                          </p>
                        )}
                        <p className="text-xs text-gray-500">
                          Created: {q.CreatedAt ? new Date(q.CreatedAt).toLocaleString() : 'Date not available'}
                        </p>
                      </div>
                    </div>
                  );
