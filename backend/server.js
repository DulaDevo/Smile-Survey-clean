...
//updated for date range
// Function to check and update active questions based on date ranges
async function updateActiveQuestions() {
  try {
    const pool = await getPool();
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    console.log('Checking for questions to activate/deactivate based on date range...');

    // Step 1: Deactivate EXPIRED questions (past ToDate)
    const deactivateExpired = await pool.request()
    ...
    console.log(`Deactivated ${deactivateExpired.rowsAffected[0]} expired questions`);

    // Step 2: Activate scheduled questions (ONLY if no active question exists)
    const activateScheduled = await pool.request()
      .input('today', sql.Date, today)
      .query(`
        WITH DepartmentsWithoutActive AS (
          ...
          AND NOT EXISTS (
            ...
            WHERE sq.DepartmentID = d.DepartmentID 
            AND sq.IsActive = 1
          )
        ),
        ScheduledQuestions AS (
          SELECT 
            sq.QuestionID,
            sq.DepartmentID,
            ROW_NUMBER() OVER (
              PARTITION BY sq.DepartmentID 
              ORDER BY sq.CreatedAt DESC
            ) as rn
          FROM SurveyQuestions sq
          INNER JOIN DepartmentsWithoutActive d ON sq.DepartmentID = d.DepartmentID
          WHERE 
            sq.IsActive = 0
            AND (sq.FromDate IS NULL OR @today >= sq.FromDate)
            AND (sq.ToDate IS NULL OR @today <= sq.ToDate)
        )
        UPDATE SurveyQuestions
        SET IsActive = 1, UpdatedAt = GETDATE()
        WHERE QuestionID IN (
          SELECT QuestionID 
          FROM ScheduledQuestions 
          WHERE rn = 1
        )
      `);

    console.log(`Activated ${activateScheduled.rowsAffected[0]} scheduled questions`);

    // Step 3: Deactivate FUTURE questions (before FromDate)
    const deactivateFuture = await pool.request()
      .input('today', sql.Date, today)
      .query(`
        UPDATE SurveyQuestions 
        SET IsActive = 0, UpdatedAt = GETDATE()
    ...
    console.log(`Deactivated ${deactivateFuture.rowsAffected[0]} future-dated questions`);

  } catch (err) {
    console.error('Error updating active questions:', err);
  }
}
...
app.post('/api/admin/trigger-question-update', async (req, res) => {
  try {
    await updateActiveQuestions();
    res.json({
      success: true,
      message: 'Question update triggered successfully'
    });
  } catch (err) {
...
app.post('/api/questions/bulk', async (req, res) => {
  try {
    const { questionText, fromDate, toDate } = req.body;
    ...
    const departmentsResult = await pool.request()
      .query('SELECT DepartmentID, Name FROM Departments WHERE IsActive = 1');
    ...
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const inDateRange = (!fromDate || new Date(fromDate) <= today) &&
      (!toDate || new Date(toDate) >= today);
    ...
    for (const dept of departments) {
      try {
        ...
        if (inDateRange) {
          // Check if department has active question
          ...
          const hasActiveQuestion = activeCheck.recordset[0].activeCount > 0;
        ...
        if (hasIsActive) {
          ...
          values += `, ${shouldBeActive ? '1' : '0'}`;
        }

        if (fromDate) {
          insertQuery += ', FromDate';
          values += ', @fromDate';
    ...
    let message = `Question added to ${departments.length} departments. `;
    if (activatedCount > 0) {
      message += `${activatedCount} activated immediately. `;
    }
    if (scheduledCount > 0) {
      message += `${scheduledCount} scheduled (will activate when current questions expire).`;
    }
...
app.post('/api/departments/:slug/questions', async (req, res) => {
  try {
    ...
    // Check if question should be active
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    let shouldBeActive = false;
    ...
    if (inDateRange) {
      ...
      const hasActiveQuestion = activeCheck.recordset[0].activeCount > 0;
      shouldBeActive = !hasActiveQuestion; // Only activate if no active question exists
    }
    ...
    res.json({
      ...
      message: shouldBeActive
        ? 'Question created and activated'
        : inDateRange
          ? 'Question created and scheduled (will activate when current question expires)'
          : 'Question scheduled for future activation'
    });
  } catch (err) {
...
app.get('/api/questions', async (req, res) => {
  try {
    ...
    if (await checkColumnExists('SurveyQuestions', 'FromDate')) {
      query += ', q.FromDate';
    }

    if (await checkColumnExists('SurveyQuestions', 'ToDate')) {
      query += ', q.ToDate';
    }
